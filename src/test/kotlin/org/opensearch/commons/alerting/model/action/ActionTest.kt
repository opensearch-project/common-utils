...
class ActionTest {
    ...
    fun `test action asTemplateArgs`() {
        ...
        assertEquals(
            action.id, templateArgs[ID_FIELD],
            "Template arg field 'id' doesn't match"
        )
        assertEquals(
            action.name, templateArgs[NAME_FIELD],
            "Template arg field 'name' doesn't match"
        )
        assertEquals(
            action.destinationId, templateArgs[DESTINATION_ID_FIELD],
            "Template arg field 'destinationId' doesn't match"
        )
        assertEquals(
            action.throttleEnabled, templateArgs[THROTTLE_ENABLED_FIELD],
