...
class ImmutableMapDeserializationTests {
    ...
    // -------------------------------------------------------------------------
    // MonitorMetadata.parse() via XContent — regression for immutable EmptyMap
    // -------------------------------------------------------------------------

    @Test
    fun `MonitorMetadata parse without last_run_context field returns mutable lastRunContext`() {
        // toXContent() omits last_run_context when empty, so a stored document has no such field.
        // Before the fix, the default was mapOf() == kotlin.collections.EmptyMap (immutable).
        // Any cast to MutableMap would throw ClassCastException.
        val json = """
            {
        ...
        val metadata = MonitorMetadata.parse(parser(json), id = "test-monitor-id-metadata")

        assertTrue(metadata.lastRunContext.isEmpty())
        assertMutableMap(metadata.lastRunContext)
    }

    @Test
    fun `MonitorMetadata parse with last_run_context field returns mutable lastRunContext`() {
        // xcp.map() without .toMutableMap() can return an immutable map when the parsed map
        // is backed by an immutable collection. Ensure the XContent assignment path is also safe.
        val json = """
            {
              "monitor_id": "test-monitor-id",
              "last_action_execution_times": [],
        ...
        assertEquals(1, metadata.lastRunContext.size)
        assertMutableMap(metadata.lastRunContext)
    }

    // -------------------------------------------------------------------------
    // Monitor.parse() via XContent — regression for immutable EmptyMap (uiMetadata)
    // -------------------------------------------------------------------------

    @Test
    fun `Monitor parse without ui_metadata field returns mutable uiMetadata`() {
        // toXContent() omits ui_metadata when empty; parse() default was mapOf() (immutable).
        val json = """
            {
              "name": "test-monitor",
              "type": "monitor",
        ...
        val monitor = Monitor.parse(parser(json))

        assertTrue(monitor.uiMetadata.isEmpty())
        assertMutableMap(monitor.uiMetadata)
    }

    @Test
    fun `Monitor parse with ui_metadata field returns mutable uiMetadata`() {
        val json = """
            {
              "name": "test-monitor",
              "type": "monitor",
              ...
              "triggers": [],
              "last_update_time": 0,
              "ui_metadata": {"layout": "graph", "version": 1}
            }
        """.trimIndent()

        val monitor = Monitor.parse(parser(json))

        assertEquals(2, monitor.uiMetadata.size)
        assertMutableMap(monitor.uiMetadata)
    }
    ...
    // -------------------------------------------------------------------------
    // BucketSelectorExtAggregationBuilder.bucketsPathsMap
    // -------------------------------------------------------------------------

    @Test
    fun `BucketSelectorExtAggregationBuilder with empty bucketsPathsMap survives serialization round-trip`() {
        val original = randomBucketSelectorExtAggregationBuilder(bucketsPathsMap = mutableMapOf())
        val out = BytesStreamOutput()
        original.writeTo(out)
        val sin = StreamInput.wrap(out.bytes().toBytesRef().bytes)
        val deserialized = BucketSelectorExtAggregationBuilder(sin)

        assertEquals(original.name, deserialized.name)
        assertTrue(deserialized.bucketsPathsMap.isEmpty())
        // Verify mutability
        assertMutableMap(deserialized.bucketsPathsMap)
    }
}
