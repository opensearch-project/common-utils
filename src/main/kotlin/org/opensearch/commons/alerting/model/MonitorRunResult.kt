...
data class InputRunResults(
    ...
    companion object {
        ...
        fun readFrom(sin: StreamInput): InputRunResults {
            ...
            val pplCount = if (sin.version.onOrAfter(Version.V_3_5_0)) {
                sin.readVInt()
            } else {
                0
            }
            val pplList = mutableListOf<Map<String, Any?>>()
            if (sin.version.onOrAfter(Version.V_3_5_0)) {
                for (i in 0 until pplCount) {
                    pplList.add(suppressWarning(sin.readMap())) // pplResults
                }
            }
            val pplNumResults = if (sin.version.onOrAfter(Version.V_3_5_0)) {
